...
public class Main : MonoBehaviour
{
    ...
    public Slider slider1; // already a variable named slider in Slider_length.cs. 
                           // Therefore, named slider1 to avoid confusion.
    ...
    {
        float length = Mathf.Sqrt(((mass.transform.position.x - pivot.transform.position.x) 
                                   * (mass.transform.position.x - pivot.transform.position.x))
                                  + ((mass.transform.position.y - pivot.transform.position.y) 
                                     * (mass.transform.position.y - pivot.transform.position.y))
                                    + ((mass.transform.position.z - pivot.transform.position.z) 
                                       * (mass.transform.position.z - pivot.transform.position.z)));
        return length;
    }
    ...
    void CalculateConstants()
    {
        ...
        series = 1 + (0.0625f * Mathf.Pow(maxAngle, 2)) + (0.0035807291667f * Mathf.Pow(maxAngle, 4)) 
                 + (0.00023464626736f * Mathf.Pow(maxAngle, 6)) + (0.000017356115674f * Mathf.Pow(maxAngle, 8)) 
                 + (0.0000013867625063f * Mathf.Pow(maxAngle, 10));
    ...
    void SetNodePos()
    {
        ...
        Vector3 mouse = new Vector3(Input.mousePosition.x, Input.mousePosition.y); //the z-coordinate is defined separately in the subsequent line.
    ...
    {
        SetNodePos();
        maxAngle = Mathf.Atan2((node.transform.position.y - pivot.transform.position.y), 
                               (node.transform.position.x - pivot.transform.position.x)) 
                   + (0.5f * Mathf.PI); //resultant angle is in radians.
        if (maxAngle > Mathf.PI) 
        { 
            maxAngle = maxAngle - (2 * Mathf.PI); 
        }
        pivot.transform.localEulerAngles = new Vector3(0, 0, maxAngle * Mathf.Rad2Deg); //Euler angles are in degrees. 
        currentTime = 0;
    }
    ...
    void Motion()
    {
        ...
        pivot.transform.localEulerAngles = new Vector3(0, 0, currentAngle * Mathf.Rad2Deg); //Euler angles are in degrees.
    }
